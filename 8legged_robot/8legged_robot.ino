...
/* apply pid */
    ...
    serpoint  = 0;
    dt    = 0.1;
    a   = accelerometer_y / 1000;
    error   = serpoint - a;
    integ   = integ + error * dt;
    diff    = (error - pre_error) / dt;
    change    = (Kp * (error + Ki * integ + Kd * diff) );
    BodyRot.Z += change;
    pre_error = error;
    delay( 100 );
  }
  ...
  if ( ( (gaitmode == 1) || (gaitmode == 2) || (gaitmode == 3) ) && (millis() - exeTime > 20000) )
/* if(((gaitmode==1)||(gaitmode==2)||(gaitmode==3))&&(abs(currDistance - distance)<10)) */
  {
/*
 *      timeSpent=millis() - exeTime;
 *       calSpeed=10/(timeSpent/1000);
 */
    calSpeed = abs( currDistance - distance ) / 10;
